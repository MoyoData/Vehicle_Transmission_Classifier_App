...
mlflow.set_tracking_uri("file:/home/chiomau/mlflow")
...
class ModelTrainer:
    ...
    def preprocess_data(self):
        ...
        for col in categorical_columns:
            logger.info(f"Encoding {col} (Unique values: {self.df[col].nunique()})")
    ...
    def split_data(self):
        ...
        if 'transmission_from_vin' not in self.df.columns:
            logger.error("'transmission_from_vin' column is missing in the dataset.")
...
if __name__ == "__main__":
    # Set the project root to one directory above the src folder
    ...
    run_name = f"classification_{config.get('model_params', {}).get('n_estimators', 'default')}{training_compute}" if config else f"classification_default{training_compute}"
